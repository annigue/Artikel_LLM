...
def call_ollama(model: str, messages: List[Dict[str, str]], temperature: float = 0.8,
                top_p: float = 0.9, repeat_penalty: float = 1.05, num_predict: int = 2048) -> str:
    payload = {
        "model": model,
        "messages": messages,
        "options": {
...
def generate_article(topic: str, details: str, primary_kw: str = "", secondary_kws: str = "",
                     model: str = "llama3.1:latest", lang: str = "de",
                     min_words: int = 700, max_words: int = 1000) -> Dict[str, Any]:
    ...
    # Pass 2: Style-Edit (Struktur & SEO beibehalten)
    edited = call_ollama(
        model,
        [
            {"role": "system", "content": system},
    ...
    while not ok and attempts < 3:
        ...
        if sm["formal_address"] > 0 or sm["second_person"] < targets["min_second_person"]:
            ...
            stc["h2_bg_tipps"], stc["h2_rezept"], stc["h3_zutaten"],
...
def main():
    parser = argparse.ArgumentParser(description="Artikel-Generator mit fester Struktur + SEO + Du-Ansprache.")
    ...
    parser.add_argument("--model", default="llama3.1:latest", help="Ollama-Modellname")
    ...
    print("# --- Humanizer Pipeline (Struktur + SEO) ---")
